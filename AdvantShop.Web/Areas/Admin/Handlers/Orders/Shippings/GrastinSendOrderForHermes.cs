...
{
    public class GrastinSendOrderForHermes
    {
        private readonly SendOrderForHermesModel _model;

        public List<string> Errors { get; set; }

        public GrastinSendOrderForHermes(SendOrderForHermesModel model)
        {
            _model = model;
        }
        ...
        public bool Execute()
        {
            ...
            if (order != null)
            {
                        ...
                        .Value)
                {
                    ...
                    var hermesOrder = new HermesOrder()
                    {
                    ...
                    if (order.OrderItems != null && order.OrderItems.Count > 0)
                    {
                        ...
                        hermesOrder.Products = orderItems.Select(x => new GrastinProduct()
                        {
                        ...
                        {
                            hermesOrder.Products.Add(new GrastinProduct()
                            {
                                ArtNo = "Доставка",
                                Name = "Доставка",
                        ...
                        hermesOrder.OrderSum = hermesOrder.Products.Sum(x => x.Price*x.Amount);
                    }
                    else
                    {
                        hermesOrder.OrderSum = order.Sum;
                    }

                    if (!_model.CashOnDelivery)
                        hermesOrder.OrderSum = 0f;

                    var response = service.AddHermesOrder(new HermesOrderContainer() { Orders = new List<HermesOrder>() { hermesOrder } });
