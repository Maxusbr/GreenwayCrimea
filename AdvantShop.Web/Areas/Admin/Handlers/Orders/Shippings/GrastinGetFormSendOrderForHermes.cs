...
{
    public class GrastinGetFormSendOrderForHermes
    {
        private readonly int _orderId;

        public GrastinGetFormSendOrderForHermes(int orderId)
        {
            _orderId = orderId;
        }

        public SendOrderForHermesModel Execute()
        {
            var order = OrderService.GetOrder(_orderId);
            if (order == null)
                return null;

            var model = new SendOrderForHermesModel()
            {
                OrderId = _orderId,
                OrderNumber = order.Number,
                Seats = 1,
                AssessedCost = order.Sum,
                Weight = order.OrderItems.Sum(x => x.Weight * x.Amount)
            };
                    ...
                    .Value)
            {
                ...
                var hermesSelfPickup = service.GetHermesSelfPickup(city);
                model.AddressPoints = hermesSelfPickup != null
                    ? hermesSelfPickup.Select(x => new SelectListItem()
                    {
                        Text = x.Name,
                        Value = x.Id,
                    }).ToList()
