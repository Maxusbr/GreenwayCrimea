...
namespace AdvantShop.Admin.Handlers.Statistics
{
    public class OrdersByHandler
    {
        ...
        public object GetShippings()
        {
            ...
            return String.Format("[ {{ 'key': '{0}', 'values': [{1}], {2} }} ]",
                "",
                shippingStat.Aggregate("",
                    (current, shipping) =>
                        current +
                        string.Format("{{ 'label': '{0}',  'value':{1}, 'count':'{2}'}},",
                            string.IsNullOrEmpty(shipping.Name) ? "n/a" : shipping.Name.Reduce(30),
                            shipping.Sum.ToInvariantString(),
                            shipping.Count)),
                _format);
        }
        ...
        private string RenderRepeatOrdersGraph()
        {
            var items = OrderStatisticsService.GetRepeatOrders(_groupFormatString, _dateFrom, _dateTo, _paied, _statusId);
            ...
            return String.Format("[{{label: '{0}', data:[{1}]}}]", Resource.Admin_Statistics_RepeatOrders, data);
        }
