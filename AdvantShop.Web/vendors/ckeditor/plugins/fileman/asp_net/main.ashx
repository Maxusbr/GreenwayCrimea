...
public class RoxyFilemanHandler : AdminHandler, IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    {
        var repStr = " ";
        var ret = filename.Replace("+", repStr)
                          .Replace("&", repStr)
                          .Replace(";", repStr)
                          .Replace(":", repStr)
                          .Replace("?", repStr)
    ...
    protected void DeleteFile(string path)
    {
        ...
        else
        {
            try
            {
                FilesStorageService.DecrementAttachmentsSize(path);
                File.Delete(path);
                _r.Write(GetSuccessRes());
            }
            catch (Exception)
        ...
        try
        {
            for (int i = 0; i < HttpContext.Current.Request.Files.Count; i++)
            {
                if (CanHandleFile(HttpContext.Current.Request.Files[i].FileName))
                {
                    ...
                    int indexFormat = f.Name.LastIndexOf(".");
                    int countStringFormat = f.Name.Length - indexFormat;
                    string formatFile = f.Name.Remove(0, indexFormat);
                    string tempName = f.Name.Remove(indexFormat, countStringFormat);
                    if (tempName.Length > 100)
                        tempName = tempName.Remove(99, tempName.Length - 100);
                    string filename = MakeUniqueFilename(path, tempName + formatFile);
                    ...
                    FilesStorageService.IncrementAttachmentsSize(filename);
                }
                else
                {
                    hasErrors = true;
