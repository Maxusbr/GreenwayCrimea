...
namespace Protractor
{
    ...
    public class NgWebElement : IWebElement, IWrapsElement
    {
        ...
        public void SendKeys(string text)
        {
            ...
            //{
            //    this.element.SendKeys(c.ToString());
            //    Thread.Sleep(1000);
            //}

            if (string.IsNullOrWhiteSpace(text) || text.Length < 3 || text.EndsWith(".jpg") || text.EndsWith(".jpeg") || text.EndsWith(".png") || text.Contains("\r\n"))
            {
                this.element.SendKeys(text);
            }
            else
            {
                var firstStr = text.Substring(0, text.Length - 1);
                this.element.SendKeys(firstStr);

                Thread.Sleep(1000);

                var lastSymbol = text.Substring(text.Length - 1);
                this.element.SendKeys(lastSymbol);
            }
        }
        ...
        public NgWebElement FindElement(By by)
        {
            ...
            this.ngDriver.WaitForAngular();
            this.ngDriver.WaitForElement(by);
