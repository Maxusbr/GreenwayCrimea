...
namespace Protractor
{
    ...
    public class NgWebDriver : IWebDriver, IWrapsDriver, IJavaScriptExecutor, ITakesScreenshot
    {
        ...
        /// The modules to load before Angular whenever Url setter or Navigate().GoToUrl() is called.
        /// </param>
        public NgWebDriver(IWebDriver driver, string rootElement, string screenshotPath,  params NgModule[] mockModules)
        {
        ...
        public string PageSource
        {
            get
            {
                this.WaitForAngular("PageSource");
        ...
        public string Url
        {
            ...
            set
            {
                ...
                if (!this.IgnoreSynchronization)
                {
                    ...
                    {
                        Console.WriteLine("Angular could not be found on the page '{0}'", value);
        ...
        public NgWebElement FindElement(By by)
        {
            ...
            this.WaitForAngular(by.ToString());
        ...
        public void WaitForAngular(string elementSelector = null)
        {
            if (!this.IgnoreSynchronization)
            {
                try
                ...
                {
                    Console.WriteLine("can't wait for angular" + (elementSelector != null ? " for element " + elementSelector : "") + " on page " + Url);
                ...
                catch(InvalidOperationException)
                {
                    ...
                    try {
                        this.ExecuteAsyncScript(ClientSideScripts.WaitForAngular, this.rootElement); // do the same again
                    }
                    catch (Exception)
                    {
                        TakeScreenshot(elementSelector + "-inner-exception");
                        throw;
                    }
                }
            }
        ...
        {
            if (by.ToString().ToLower().Contains("cke"))
                return;

            try
            {
                WebDriverWait wait = new WebDriverWait(driver, TimeSpan.FromSeconds(10));
                IWebElement elem = wait.Until<IWebElement>(ExpectedConditions.ElementExists(by));
            } catch (WebDriverTimeoutException)
            {
                Console.WriteLine("element " + by.ToString() + " not found");
        ...
        private void TakeScreenshot(string element)
        {
            ITakesScreenshot screenshotDriver = driver as ITakesScreenshot;
            Screenshot screenshot = screenshotDriver.GetScreenshot();

            var dirPath = Directory.GetDirectories(screenshotPath).LastOrDefault();
            if (dirPath == null)
            {
                dirPath = screenshotPath + "temp";
                Directory.CreateDirectory(dirPath);
            }
            dirPath += "\\";

            var filePath = dirPath + ((driver.Url.Length > 100 ? driver.Url.Substring(0, 99) : driver.Url) + "-el-" + element).Replace("/", "-").Replace("?", "@").Replace(":", "$").Replace("\"", "$").Trim('-');
            ...
            using (TextWriter tw = new StreamWriter(file + ".html"))
            {
                tw.Write(driver.PageSource);
            }
        ...
        {
            return (this.driver as ITakesScreenshot).GetScreenshot();
        }
    }
}
