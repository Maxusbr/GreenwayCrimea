...
function checkApp()
{
    return document.getElementsByTagName('html')[0].getAttribute('data-ng-app') != null || document.getElementsByTagName('html')[0].getAttribute('ng-app')  != null;
}
";
...
if(checkApp()){
    if (window.angular && !(window.angular.version && window.angular.version.major > 1)) {
        /* ng1 */
        getNg1Hooks(rootSelector, undefined, function(hooks){
       ...
       console.warn('Using protractor on non-angular page');
...
callback();
         ...
         */
        public const string TestForAngular = GetNg1HooksHelper +  @"
if(checkApp())
{
    ...
    var check = function() {
        ...
        if (definitelyNg1 && window.angular) {
            console.warn('angular never provided resumeBootstrap');
        } else {
...
window.__TESTABILITY__NG1_APP_ROOT_INJECTOR__ = 
    angular.resumeBootstrap(arguments[0].length ? arguments[0].split(',') : []);";
        ...
        public const string GetLocation = GetNg1HooksHelper + @"
            getNg1Hooks(arguments[0], undefined, function(hooks){
        ...
        public const string SetLocation = GetNg1HooksHelper + @"
            var hooks = getNg1Hooks(arguments[0], undefined, function(hooks){
                var url = arguments[1];
                if (angular.getTestability) {
