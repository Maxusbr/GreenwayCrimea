    ...
    public class ExcelExport
    {
        ...
        private static void RenderOrderItems(ExcelWorksheet worksheet, Order order)
        {
            ...
            foreach (OrderItem item in order.OrderItems)
            {
                ...
                if (i != 1)
                {
                    worksheet.InsertRow(21, 1, 20); //shift down
                                                    //worksheet.Row(19 + i).StyleID = worksheet.Row(20).StyleID;
                }

                worksheet.Cells[i != 1 ? 21 : 20, 1].Value = string.Format("{0}, {1}", item.ArtNo, item.Name);
                ...
                worksheet.Cells[i != 1 ? 21 : 20, 3].Value = PriceFormatService.FormatPrice(item.Price, order.OrderCurrency);
                worksheet.Cells[i != 1 ? 21 : 20, 4].Value = item.Amount.ToString();
                worksheet.Cells[i != 1 ? 21 : 20, 5].Value = PriceFormatService.FormatPrice(item.Price * item.Amount, order.OrderCurrency);
        ...
        public static void SingleOrder(string templatePath, string filename, Order order)
        {
            using (var excel = new ExcelPackage(new System.IO.FileInfo(templatePath)))
            {
                ...
                worksheet.Cells[12, 1].Value = "     " + LocalizationService.GetResource("Admin.Orders.OrderItem.City") + order.OrderCustomer.City;
                worksheet.Cells[12, 2].Value = "     " + LocalizationService.GetResource("Admin.Orders.OrderItem.City") + order.OrderCustomer.City;
                ...
                worksheet.Cells[17, 1].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.OrderItem");

                worksheet.Cells[19, 1].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.ItemName");
                worksheet.Cells[19, 2].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.CustomOptions");
                worksheet.Cells[19, 3].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.Price");
                worksheet.Cells[19, 4].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.ItemAmount");
                worksheet.Cells[19, 5].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.ItemCost");

                //productprice
                var currency = order.OrderCurrency;
                float productPrice = order.OrderItems.Sum(item => item.Amount * item.Price);
                worksheet.Cells[21, 4].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.ProductsPrice");
                worksheet.Cells[21, 5].Value = PriceFormatService.FormatPrice(productPrice, currency);

                int summaryRow = 22;
                int styleRow = 22;
                //totalsum
                worksheet.Cells[summaryRow, 4].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.TotalPrice");
                worksheet.Cells[summaryRow, 5].Value = PriceFormatService.FormatPrice(order.Sum, currency);

                //comment
                worksheet.Cells[23, 1].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.CustomerComment");
                worksheet.Cells[24, 1].Value = order.CustomerComment;
                ...
                worksheet.Cells[summaryRow, 4].Value = "(" + order.ArchivedPaymentName + ")";

                worksheet.InsertRow(summaryRow, 1, styleRow);
                worksheet.Cells[summaryRow, 4].Value = LocalizationService.GetResource("Core.ExportImport.ExcelSingleOrder.PaymentExtracharge");
                worksheet.Cells[summaryRow, 5].Value = "+" + PriceFormatService.FormatPrice(order.PaymentCost, currency);

                //}

                worksheet.InsertRow(summaryRow, 1, styleRow);
                worksheet.Cells[summaryRow, 4].Value = "(" + shippingMethodName + ")";

                worksheet.InsertRow(summaryRow, 1, styleRow);
                worksheet.Cells[summaryRow, 4].Value = LocalizationService.GetResource("Core.ExportImport.ExcelSingleOrder.ShippingPrice");
                worksheet.Cells[summaryRow, 5].Value = "+" + PriceFormatService.FormatPrice(order.ShippingCost, order.OrderCurrency);
                ...
                if (taxedItems.Count > 0)
                {
                    ...
                    {
                        worksheet.InsertRow(summaryRow, 1, styleRow);
                        worksheet.Cells[summaryRow, 4].Value = (tax.ShowInPrice ? LocalizationService.GetResource("Core.Tax.IncludeTax") + " " : "") + tax.Name + ":";
                        worksheet.Cells[summaryRow, 5].Value = (tax.ShowInPrice ? "" : "+") + PriceFormatService.FormatPrice(taxedItems[tax], currency);
                    }
                }
                else
                {
                    worksheet.InsertRow(summaryRow, 1, styleRow);
                    worksheet.Cells[summaryRow, 4].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.Taxes");
                    worksheet.Cells[summaryRow, 5].Value = "+" + PriceFormatService.FormatPrice(0, currency);
                }
                ...
                {
                    worksheet.InsertRow(summaryRow, 1, styleRow);
                    worksheet.Cells[summaryRow, 4].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.Bonuses");
                    worksheet.Cells[summaryRow, 5].Value = "-" + PriceFormatService.FormatPrice(bonusPrice, currency);
                }

                if (order.Certificate != null)
                {
                    worksheet.InsertRow(summaryRow, 1, styleRow);
                    worksheet.Cells[summaryRow, 4].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.Certificate");
                    worksheet.Cells[summaryRow, 5].Value = "-" + string.Format("-{0}", PriceFormatService.FormatPrice(order.Certificate.Price, currency));
                }
                ...
                if (order.OrderDiscount != 0 || order.OrderDiscountValue != 0)
                {
                    ...
                    worksheet.InsertRow(summaryRow, 1, styleRow);
                    worksheet.Cells[summaryRow, 4].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.Discount");
                    worksheet.Cells[summaryRow, 5].Value = "-" +
                ...
                if (order.Coupon != null)
                {
                    //insert before summaryRow row with copy style from styleRow
                    worksheet.InsertRow(summaryRow, 1, styleRow);
                    worksheet.Cells[summaryRow, 4].Value = LocalizationService.GetResource("Admin.Orders.OrderItem.Coupon");
                    ...
                    switch (order.Coupon.Type)
                    {
                        case CouponType.Fixed:
                            worksheet.Cells[summaryRow, 5].Value = string.Format("-{0} ({1})",
                                                                        PriceFormatService.FormatPrice(order.Coupon.Value, currency),
                                                                        order.Coupon.Code);
                            break;
                        case CouponType.Percent:
                            worksheet.Cells[summaryRow, 5].Value = string.Format("-{0} ({1}%) ({2})",
